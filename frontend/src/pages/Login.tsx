  ...
  return (
     <Box width={'100%'} height={'100%'} display={'flex'} flex={1}>
          <Box padding={8} mt={8} display={{md:'flex',sm:'none',xs:'none'}}>
             <img src="aiRobot2.jpg" className='image-with-background' alt="Robot" style={{width:'400px'}} />{/*change this image to the ai robot so it can look way cooler*/}
          </Box>
          <Box display={'flex'} flex={{xs:1,md:0.5}} justifyContent={'center'} alignItems={'center'} padding={2} ml={'auto'} mt={16}>
            <form onSubmit={e => handleSubmit(e)} style={{margin:'auto',padding:'30px',boxShadow:'10px 10px 20px #000',borderRadius:'10px',border:'none'}}>
              <Box sx={{display:'flex',flexDirection:'column',justifyContent:'center',gap:2}}>
                    ...
                    <Button type='submit' endIcon={<IoIosLogIn/>} sx={{px:2,py:1,mt:2,width:'400px',borderRadius:2,bgcolor:'#00fffc',':hover':{
